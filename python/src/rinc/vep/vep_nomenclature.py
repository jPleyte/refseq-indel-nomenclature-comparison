...
class VepNomenclature(object):
    ...
    def _get_variant_values(self, row: pd.Series):
        ...
        """
        chromosome, position = row['Location'].split(':')
        alt = row['Allele']
        transcript = row['Feature']
        
        g_chr, g_pos, g_ref, g_alt = self._get_variant_values_from_gdot(row['HGVSg'])
        
        if 'GIVEN_REF' in row: 
            reference = row['GIVEN_REF']
        else:
            reference = g_ref
        
        if chromosome != g_chr or position != g_pos or g_ref != reference or  g_alt != alt:
            raise ValueError(f"HGVSg does not match variant parts: {row['HGVSg']} != {chromosome}-{position}-{reference}-{alt}") 
        
        return chromosome, position, reference, alt, transcript
    
    def _get_variant_values_from_gdot(self, hgvs_g: str):
        """
        Parse the variant values out of a HGVSg field (eg 12:g.9994422G>A) 
        """
        pattern = r"(\w+):g\.(\d+)([A-Z]+)>([A-Z]+)"
        match = re.match(pattern, hgvs_g)
...
def _parse_args():
    ...
    parser.add_argument("--vep_resuls", help="File generated by VEP (tsv)", required=True)
    parser.add_argument("--out", help="nomenclature output file (csv)", dest="output", required=True)
...
def main():
    ...
    vn.read_vep_file(args.vep_resuls)
